...
fn parse_and_solve_empty_formula() {
    ...
    match formula {
        Err(_) => assert!(false),
        Ok(formula) => assert_eq!(solver.solve(formula), SATResult::SAT(HashMap::new())),
    };
}
...
fn parse_and_solve_formula_with_only_empty_clause() {
    ...
    match formula {
        Err(_) => assert!(false),
        Ok(formula) => assert_eq!(solver.solve(formula), SATResult::UNSAT),
    };
}
...
fn parse_and_solve_formula_with_only_a_unit_clause() {
    ...
    match formula {
        Err(_) => assert!(false),
        Ok(formula) => assert_eq!(solver.solve(formula), SATResult::SAT(soln)),
    };
}
...
fn parse_and_solve_formula_with_one_non_unit_clause() {
    ...
    match formula {
        Err(_) => assert!(false),
        Ok(formula) => match solver.solve(formula) {
